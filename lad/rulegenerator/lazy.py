...
class LazyPatterns:
                ...
                while len(attributes) > 1:
                    ...
                    for att in attributes:
                        ...
                        if (
                            ...
                            or (
                                __confidence == confidence and (__support > support)
                                if support and __support
                                else False
                            )
                            or (
                                __confidence == confidence
                                and __support == support
                                and (__lift > lift)
                                if lift and __lift
                                else False
                            )
    ...
    def __adjust(self):
        for r in self.__rules:
            conditions = {}
            cutpoints = [
                self.__binarizer.get_cutpoints()[i]
                for i in self.__selector.get_selected()[r["attributes"]]
            ]
    ...
    def predict_proba(self, X):
        ...
        output = np.zeros((len(X), len(np.unique(self.__y if self.__y else []))))
    ...
    def __get_stats(self, instance, attributes, label):
        covered = np.where(
            (
                self.__Xbin[:, attributes]
                if self.__Xbin
                else None == instance[attributes]
            ).all(axis=1)
        )
        ...
        if len(covered[0]) > 0:
            unique, counts = np.unique(
                self.__y[covered] if self.__y else [], return_counts=True
            )
            ...
            if label in unique:
                ...
                support = (
                    (counts[argmax] / self.__Xbin.shape[0]) if self.__Xbin else None
                )
                lift = (
                    (
                        (counts[argmax] / len(covered[0]))
                        / (self.__labels[unique[argmax]] / self.__y.shape[0])
                    )
                    if self.__y
                    else None
                )
